   ...
   // g.displayMap();
    ...
    void displayMap() {
        ...
        for (const auto& station : stations) {
            ...
            for (const auto& connection : station.second) {
                cout << "\t" << connection.first << "\t" << connection.second << " km" << endl;
            }
            cout << "-------------------------------------" << endl;
        }
    }
    ...
    vector<string> getShortestPath(const string& source, const string& destination) {
        ...
        while (!pq.empty()) {
            ...
            for (const auto& connection : stations[current]) {
                string adjacent = connection.first;
                ...
                if (distance[current] + weight < distance[adjacent]) {
                    ...
                    pq.push(make_pair(distance[adjacent], adjacent));
                }
            }
        }
...
int main() {
    ...
    // Add connections
    metroMap.addConnection("Noida Sector 62", "Botanical Garden", 8);
    metroMap.addConnection("Botanical Garden", "Yamuna Bank", 10);
    metroMap.addConnection("Yamuna Bank", "Vaishali", 8);
    metroMap.addConnection("Yamuna Bank", "Rajiv Chowk", 6);
    metroMap.addConnection("Rajiv Chowk", "Moti Nagar", 9);
    metroMap.addConnection("Moti Nagar", "Janak Puri West", 7);
    metroMap.addConnection("Janak Puri West", "Dwarka Sector 21", 6);

    // Display the metro map
    metroMap.displayMap();

    // Get shortest path
    string source = "Noida Sector 62";
    string destination = "Dwarka Sector 21";
    vector<string> path = metroMap.getShortestPath(source, destination);
